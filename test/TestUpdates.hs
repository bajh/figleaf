...
import Test.HUnit
import Commands(aParse)
import Tree(
    Tree(..)
    , tUpdate
    , Assignment(..))

updateTest d expT inA inT = TestCase (assertEqual d expT (tUpdate inA inT))

testUpdate1 = updateTest
    "for tree with just leaf node"
    (Tree "default" [Leaf "DB_NAME" "users"])
    (Assignment ["DB_NAME"] "users")
    (Tree "default" [Leaf "DB_NAME" "people"]) 
...
testUpdate4 = updateTest
    ...
    (Tree "default" [Tree "STG" [], Leaf "CERT" "./cert", Tree "PRD" [Leaf "api_key" "123"]])
    (Assignment ["PRD", "api_key"] "123")
    (Tree "default" [Tree "STG" [], Leaf "CERT" "./cert", Tree "PRD" []]) 
...
tests = TestList [ TestLabel "updating tree 1" testUpdate1
    ...
    , TestLabel "parsing assignment" testParseAssignment1
    , TestLabel "parsing invalid assignment" testParseAssignmentErr ]